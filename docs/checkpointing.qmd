...
::: callout-important
## Re-pass your checkpoint config on retry

The `--checkpoint` configuration is not recorded in the `.eval` log, so `inspect eval-retry` starts from the default. With default configuration the retry still finds and restores your checkpoints — but any options you set are dropped. Pass the same `--checkpoint` you used originally:

``` bash
inspect eval arc.py --checkpoint=cfg.yaml   # CRASH!
inspect eval-retry logs/<log-file-name>.eval --checkpoint=cfg.yaml
```

This matters most for `checkpoints_location` (without it the retry looks in the default location and finds nothing) and for `restore_usage` (without it the resumed sample silently restarts its budget at zero).
:::
...
4.  Usage (opt-in): with `restore_usage=True`, the sample's token, cost, turn, time, and working-time counters continue from the checkpoint instead of restarting at zero. Off by default. The message limit always continues, since it counts the restored conversation. Exception: on a scoring-phase resume (agent already finished; only scoring re-runs), the log still reports cumulative `total_time` and `working_time`, but the time and working-time *limits* are not shortened by prior usage — seeding an already-spent clock there could cancel the restore itself.
...
Whether to enable `restore_usage` is a trade-off. Left off, a sample that crashes repeatedly can spend a multiple of its configured budget — each attempt gets the full allowance. Turned on, a sample can resume with almost nothing left and fail immediately, turning a recoverable crash into a lost sample. Off is the default because checkpointing's primary job is resilience.

Usage is restored as of the checkpoint, not as of the crash: work done after the last checkpoint is rolled back, so it is not charged. Time spent restoring *is* charged to the time and working limits.
