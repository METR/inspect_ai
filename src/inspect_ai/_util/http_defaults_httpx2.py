...
try:
    ...
    from httpx2._utils import get_environment_proxies as _get_environment_proxies
...
def max_tokens_timeout(
    max_tokens: int | None, base: httpx2.Timeout | float | None
) -> httpx2.Timeout | None:
    ...
    if base is None:
        return None
    base_timeout = base if isinstance(base, httpx2.Timeout) else httpx2.Timeout(base)
...
async def _floor_connect_timeout(request: httpx2.Request) -> None:
    ...
    if not isinstance(extension, dict):
        return
    # Safe to mutate: httpx2.Timeout.as_dict() builds this fresh per request.
...
def default_client_kwargs(**overrides: Any) -> dict[str, Any]:
    """`httpx2.AsyncClient` kwargs carrying these defaults; caller overrides win."""
...
def default_async_client(**overrides: Any) -> httpx2.AsyncClient:
    return httpx2.AsyncClient(**default_client_kwargs(**overrides))
