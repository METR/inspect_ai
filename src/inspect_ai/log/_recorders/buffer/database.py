...
def sync_to_filestore(
    ...
    for manifest_sample in manifest.samples:
        # cursor is the max of each last_*_id across this sample's segments.
        # a segment's last_*_id only reflects items added in that segment, so
        # a segment with no new items of a given type leaves that field at 0;
        # using only the last segment would reset the cursor and re-include
        # already-written items in the next segment.
        after_event_id = 0
