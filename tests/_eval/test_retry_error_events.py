...
def test_buffer_provider_iter_events_streams_first_event_before_later_rows() -> None:
    ...
    class RaisingHistory:
        def __init__(self) -> None:
            ...
            self.events_data: EventsData = {"messages": [], "calls": []}
    ...
    class FakeBufferDb:
        ...
        def sample_event_count(self, sample_id: str | int, epoch: int) -> int:
            return 1

        def sample_attachment(
            self, sample_id: str | int, epoch: int, hash: str
        ) -> str | None:
        ...
        def import_checkpoint_events(
            ...
            sample_id: str | int,
            epoch: int,
            transcript_store: SampleEventHistorySink,
        ) -> int:
            raise NotImplementedError

        def open_sample_history_tail(
            self, sample_id: str | int, epoch: int, n: int
        ) -> AbstractContextManager[SampleHistoryLike]:
            raise NotImplementedError

        def open_sample_history_from(
            self, sample_id: str | int, epoch: int, start: int
        ) -> AbstractContextManager[SampleHistoryLike]:
            raise NotImplementedError

    provider = BufferTranscriptHistoryProvider(FakeBufferDb(), "sample", 0)
