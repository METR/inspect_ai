...
class _Transcript(SimpleNamespace):
    history: SimpleNamespace

    @property
    def event_count(self) -> int:
        raise AssertionError("textual transcript view should use history.event_count")

    @property
    def resident_events(self) -> Sequence[Event]:
        raise AssertionError(
            "textual transcript view should use history.resident_events"
        )
...
class _Sample:
    ...
    def __init__(self, transcript: _Transcript) -> None:
...
async def test_textual_transcript_view_uses_resident_events(monkeypatch) -> None:
    ...
    resident_events = [sample_init, resident]
    ...
    with pytest.raises(AssertionError, match="history.event_count"):
        _ = transcript.event_count
    with pytest.raises(AssertionError, match="history.resident_events"):
        _ = transcript.resident_events
